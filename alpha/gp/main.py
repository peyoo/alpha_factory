...
# ==========================
# !!! 非常重要。给deap打补丁
from alpha.gp.cs.deap_patch import *  # noqa
from alpha.gp.cs.base import print_population, population_to_exprs, filter_exprs, strings_to_sympy  # noqa
# ==========================
# TODO 单资产多因子，计算时序IC,使用gp_base_ts
# TODO 多资产多因子，计算截面IC,使用gp_base_cs
from alpha.gp.cs.custom import add_constants, add_operators, add_factors, RET_TYPE
...
def map_exprs(evaluate, invalid_ind, gen, label, split_date):
    ...
    # 读取历史fitness
    try:
        with open(LOG_DIR / 'fitness_cache.pkl', 'rb') as f:
    ...
    if len(exprs_list) > 0:
        ...
        # 保存适应度，方便下一代使用
        with open(LOG_DIR / 'fitness_cache.pkl', 'wb') as f:
