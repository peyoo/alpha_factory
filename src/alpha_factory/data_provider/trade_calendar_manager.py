...
class TradeCalendarManager:
        ...
        if self.path.exists():
            try:
                ...
                if self._df.height > 0:
                    logger.debug(
                        f"✓ 交易日历加载成功 (起始: {self.start_date_str}, 截止: {self._df['date'].max()})"
                    )
    ...
    def _refresh_cache_locked(self):
        ...
        if self._df.height > 0:
            self._trade_days_cache = (
                self._df.filter(pl.col("is_open") == 1).get_column("date").sort()
            )
    ...
    def sync_from_tushare(self, force: bool = False, days_buffer: int = 10) -> None:
        ...
        try:
            ...
            logger.info(
                f"正在从 Tushare 同步全量交易日历 (起始: {self.start_date_str})..."
            )

            df_raw = pro.trade_cal(
                exchange="SSE", start_date=self.start_date_str, end_date="20351231"
            )
    ...
    def offset(self, date_val: datetime.date, n: int) -> Optional[datetime.date]:
        ...
        with self._lock:
            ...
            if n >= 0:
                ...
                if (
                    idx >= self._trade_days_cache.len()
                    or self._trade_days_cache[idx] != date_val
                ):
                    idx -= 1
                target_idx = idx + n + 1
