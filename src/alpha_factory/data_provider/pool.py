...
class PoolUniverse:
    ...
    def pool(self, lf: pl.LazyFrame) -> pl.LazyFrame:
        """
        静态过滤:比如板块，一只股票如果在需要的整个生命周期都可以过滤，那就应该静态过滤
        动态过滤：一只股票在不同的时刻不断进出股票池，就应该使用股票池掩码F.POOL_MASK
        :param lf:
        :return:
        """
        return lf
...
class MainSmallPool(PoolUniverse):
    ...
    def pool(
        ...
        Schema:
            输入：必须包含列 DATE, ASSET, EXCHANGE, MARKET_TYPE, TOTAL_MV,
        ...
        return (
            ...
            .with_columns(
                [
                    ...
                    # 先算 RET
                    (pl.col(F.CLOSE) / pl.col(F.CLOSE).shift(1) - 1)
                    .over(F.ASSET)
                    .alias("RET"),
