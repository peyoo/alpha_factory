...
def label_OO_for_IC(
    lf: pl.LazyFrame, label_window=1, mask_col=F.POOL_MASK
) -> pl.LazyFrame:
    """
        ...
        [
            (
                (
                    ...
                    / pl.col("OPEN").shift(-1).over(F.ASSET)
                )
                - 1
            ).alias(label_y)
        ]
...
def label_OO_for_tradable(
    ...
    lf = lf.with_columns(
        [
            ...
            # 获取 T+1 日（买入日）是否一字涨停
            (
                pl.col("LOW").shift(-1).over("ASSET")
                == pl.col("HIGH").shift(-1).over("ASSET")
            ).alias("_is_limit_T1"),
            # 获取 T+1 日（买入日）是否触达涨停 (简单逻辑：收盘涨幅 > 9.8%)
            (
                (
                    pl.col("CLOSE").shift(-1).over("ASSET")
                    / pl.col("CLOSE").over("ASSET")
                    - 1
                )
                > 0.098
            ).alias("_at_limit_T1"),
